...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        print("Image to text: %.1f %.1f %.1f %.1f %.1f R_P: %.1f mAP: %.1f %.1f %.1f" % r)
        print("Average t2i Recall: %.1f" % ari)
        print("Text to image: %.1f %.1f %.1f %.1f %.1f R_P: %.1f mAP: %.1f %.1f %.1f" % ri)
    else:
        ...
        print("Image to text: %.1f %.1f %.1f %.1f %.1f R_P: %.1f mAP: %.1f %.1f %.1f" %
              mean_metrics[:9])
        print("Average t2i Recall: %.1f" % mean_metrics[12])
        print("Text to image: %.1f %.1f %.1f %.1f %.1f R_P: %.1f mAP: %.1f %.1f %.1f" %
              mean_metrics[9:18])
...
def i2t(images, captions, caplens, sims, npts=None, return_ranks=False):
    ...
    for index in range(npts):
        ...
        rank = 1e20
        spe_rank = np.zeros(inds.shape)
        for i in range(5 * index, 5 * index + 5, 1):   
            tmp = np.where(inds == i)[0][0]            
            if tmp < rank:
                rank = tmp
            spe_rank[np.where(inds == i)[0][0]]=1
        rs.append(spe_rank[:5])
        ranks[index] = rank
        top1[index] = inds[0]
    # Compute metrics
    ...
    r30 = 100.0 * len(np.where(ranks < 30)[0]) / len(ranks)
    medr = np.floor(np.median(ranks)) + 1
    meanr = ranks.mean() + 1
    mAP = 100.0 * mean_average_precision(rs)
    r_p = 100.0 * np.mean([r_precision(r) for r in rs])
    if return_ranks:
        return (r1, r5, r10, r20, r30, r_p, mAP, medr, meanr), (ranks, top1)
    else:
        return (r1, r5, r10, r20, r30, r_p, mAP, medr, meanr)
...
def t2i(images, captions, caplens, sims, npts=None, return_ranks=False):
    ...
    for index in range(npts):    
        for i in range(5):       
            ...
            spe_rank = np.zeros(inds.shape)               
            spe_rank[np.where(inds == index)[0][0]]=1     
            rs.append(spe_rank[:1])
